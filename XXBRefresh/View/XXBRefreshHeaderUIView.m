...
@interface XXBRefreshHeaderUIView ()
{
...
@implementation XXBRefreshHeaderUIView
...
- (void)setLastUpdateTime:(NSDate *)lastUpdateTime {
    ...
    // 1.归档
    [[NSUserDefaults standardUserDefaults] setObject:lastUpdateTime forKey:XXBRefreshHeaderUIViewLastRefreshTime];
...
- (NSDate *)lastUpdateTime {
    if (_lastUpdateTime == nil) {
        _lastUpdateTime = [[NSUserDefaults standardUserDefaults] objectForKey:XXBRefreshHeaderUIViewLastRefreshTime];
    }
...
- (void)setRefreshState:(XXBRefreshState)refreshState {
    ...
    switch (refreshState) {
        case XXBRefreshStateDefault:
        {
            ...
            self.messageLabel.text = XXBRefreshDropDown;
            // 执行动画
